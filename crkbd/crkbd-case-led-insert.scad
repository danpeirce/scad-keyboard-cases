...
module crkbd_top_case(raised = raised) {
    ...
    difference() {
        union() {
            ...
            translate([148.1, -70.6, -8.2]) cylinder(h=30-19, r1=3.8/2, r2=5.5/2, center=false, $fn=30); // pico spacer
            translate([148.1-11.4, -70.6, -8.2]) cylinder(h=30-19, r1=3.8/2, r2=5.5/2, center=false, $fn=30);
            translate([148.1, -70.6+47, -8.2]) cylinder(h=30-19, r1=3.8/2, r2=5.5/2, center=false, $fn=30);
            translate([148.1-11.4, -70.6+47, -8.2]) cylinder(h=30-19, r1=3.8/2, r2=5.5/2, center=false, $fn=30);
            
        }
        translate([148.1, -70.6, -26]) cylinder(h=30, r=2.2/2, center=false, $fn=30); // holes mount pico
        translate([148.1-11.4, -70.6, -26]) cylinder(h=30, r=2.2/2, center=false, $fn=30);
        translate([148.1, -70.6+47, -26]) cylinder(h=30, r=2.2/2, center=false, $fn=30);
        translate([148.1-11.4, -70.6+47, -26]) cylinder(h=30, r=2.2/2, center=false, $fn=30);
        translate([148.1, -70.6+26, -26]) cylinder(h=30, r=2.2/2, center=false, $fn=30);  // holes mount TRRS connector
        translate([148.1-11.4, -70.6+26, -26]) cylinder(h=30, r=2.2/2, center=false, $fn=30);
        translate([148.1, -70.6+26-20, -26]) cylinder(h=30, r=2.2/2, center=false, $fn=30);
        translate([148.1-11.4, -70.6+26-20, -26]) cylinder(h=30, r=2.2/2, center=false, $fn=30);
        translate([148.1-11.4/2, -70.6+26-20/2, -26]) cylinder(h=30, r=6/2*1.4, center=false, $fn=4); // hole pass wires
...
if (part == "outer") { // To preview the outer profile, key hole, and screw hole positions
    ...
    difference() {    
        union() {
            difference() {
                translate([12.8, 52.3, -4.3]) cube([19.5, 20, 4]); 
                translate([12.5, 52.3+3.2, -4.3-0.5]) cube([4, 2.4, 6]);  // bolt slots
                translate([12.5, 52.3+3.2+11.4, -4.3-0.5]) cube([4, 2.4, 6]);
                translate([12.5+11.4, 52.3+3.2+1.75, -4.3-0.5]) cube([5, 3, 6]);  // switch access hole
            }
            translate([12.5+11.4+5.2, 52.3+3.2+1.7-2.9, -4.3-0.5+4]) cube([3, 5, 6]);  
        }
        translate([12.5+11.4+5.4, 52.3+3.2, -4.3-0.5]) rotate([0, 15, 0]) cylinder(h=15, d1=2.1, d2=1.95, center=false, $fn=40);
        translate([12.8+5, 52.3-1, -4.3+2]) cube([5, 22, 4]);
        translate([12.8+9.5, 52.3-1+11, -4.3+2.1]) cube([5, 22, 4]);
    }
    
    
    //translate([12.8, 62.3, 13]) rotate([0,180,0]) color("blue") pico();
