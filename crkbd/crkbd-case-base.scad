...
module crkbd_bottom_case(raised = raised) {
    translate([0,0,3.0+0.01]) 
              screw_positions(crkbd_screw_holes) cylinder(r= screw_rad+0.3+1, h = 0.2, $fn=8); // sacrificial
    difference() {
        union() {
            ...
            translate([0, 0, -0.1]) screw_positions(crkbd_screw_holes) cylinder(r = 6/2+0.3, h = 3.1, $fn=6);
    }
}
